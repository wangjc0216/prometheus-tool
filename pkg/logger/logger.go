...
func Init(fn string, level string, app string, stdPrint bool) {
	once.Do(func() { initHelper(fn, level, app, stdPrint) })
}

func initHelper(fn string, level string, app string, stdPrint bool) {
	...
	if stdPrint {
		syncer = zapcore.NewMultiWriteSyncer(zapcore.AddSync(os.Stdout), zapcore.AddSync(hook)) // 打印到控制台和文件
	} else {
		syncer = zapcore.AddSync(hook) // 打印到文件
	}
...
func init() {
	initHelper("start.log", "debug", "", true)
}
